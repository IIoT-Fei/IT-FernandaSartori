...
from wtforms.validators import DataRequired, StopValidation, Length, EqualTo
from wtforms.widgets import TextArea

from app.validators.validators import NotExistsInDb
...
class Novadisciplina(FlaskForm):
    ...
    chave = PasswordField("chave", validators=[DataRequired(message="Chave requerida!"), Length(min=6, max=25,message="Sua senha deve ter no mínimo 6 caracteres e no máximo 25!"),EqualTo("conf_chave", message="As senhas devem ser correspondentes!")])
    xnt = "png jpg "
...
class NovoLab(FlaskForm):
    ...
    arquivo = FileField(validators=[FileAllowed(xnt.split(" "), "As extensões permitidas são somente: " + xnt), FileRequired("Arquivo requerido!!!")])
    visivel = BooleanField("visivel")
    data = DateTimeField(validators=[DataRequired("Data requerida")],format='%Y-%m-%d %H:%M:%S')
